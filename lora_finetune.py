# Attribution: this script builds upon the `.py` scripts in the [GitHub StyleTTS2 Repository](https://github.com/yl4579/StyleTTS2/).
# load packages
import random
import yaml
import time
...
@click.option('--lora_alpha', default=128, type=int)
@click.option('--lora_path', default='', type=str)
@click.option('--start', default=0, type=int, help='Starting epoch number')
def main(config_path, lora_r, lora_alpha, lora_path, start):
    ...
    # ---- LoRA Integration Start ----
    from peft import LoraConfig, inject_adapter_in_model
    from peft.tuners.lora.layer import LoraLayer

    # Decide which models to fully finetune
    full_finetune_model_names = ['mpd', 'msd', 'wd', 'text_aligner']

    # Decide which models to keep static (no LoRA)
    static_model_names = ['pitch_extractor', 'text_encoder']

    # Freeze all parameters except those in full_finetune modules
    for model_name, m in model.items():
        if model_name not in full_finetune_model_names:
            m.requires_grad_(False)

    # Inject LoRA adapters into models that are not static and not fully finetuned
    for model_name, m in model.items():
    ...
    for model_name, m in model.items():
        ...
        print(
            f"{model_name} || trainable params: {m_trainable_params:,d} || all params: {m_total_params:,d} || trainable%: {100 * m_trainable_params / m_total_params if m_total_params>0 else 0}"
        )
    print(
        f"total || trainable params: {trainable_params:,d} || all params: {total_params:,d} || trainable%: {100 * trainable_params / total_params if total_params>0 else 0}"
    )
    # ---- LoRA Integration End ----
    ...
    start_epoch = start # set this value to the epoch number with which you want to start the training 
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            if epoch >= joint_epoch:
                ...
                if slm_out is not None:
                    ...
                    total_norm = {}
                    for key_m in model.keys():
                        total_norm[key_m] = 0
                        parameters = [p for p in model[key_m].parameters() if p.grad is not None and p.requires_grad]
                        for p in parameters:
                            param_norm = p.grad.detach().data.norm(2)
                            total_norm[key_m] += param_norm.item() ** 2
                        total_norm[key_m] = total_norm[key_m] ** 0.5

                    # gradient scaling
                    if total_norm['predictor'] > slmadv_params.thresh:
                        for key_m in model.keys():
                            for p in model[key_m].parameters():
